...
def _run_live(args: argparse.Namespace) -> int:
    ...
    tile_bounds = None
    try:
        config = json.loads(Path("calibration_config.json").read_text(encoding="utf-8"))
        tile_bounds = config.get("tiles")
        if tile_bounds:
            print(f"已加载 {len(tile_bounds)} 张牌切图边界")
    except Exception:
        pass
    ...
    try:
        while True:
            ...
            # 识别
            result: RecognitionResult = recognize_tiles(hand_frame, river_frame, templates=templates, tile_bounds=tile_bounds)
